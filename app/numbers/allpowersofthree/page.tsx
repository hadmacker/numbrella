...
export default function Page() {
  ...
  for (let i = 0; i <= n; i++) {
    const answer = Math.pow(3, i);
    divElements.push(
      <div key={i}>
        {Formula(i, 3, answer, false)}
      </div>
    );
  }
  ...
  return (
    <>
      <span className="mx-3 md:mx-2 md:p-1 text-2xl md:text-3xl font-black text-gray-500">List of Powers of Three</span>
...
function Formula(power: number, base: number, answer: number, bw: boolean) {
  return (
    <>
      <p className="text-left m-5 tracking-wide font-mono text-5xl font-black">
        {formatted(base, bw)}
        <sup>{formatted(power, bw)}</sup>
